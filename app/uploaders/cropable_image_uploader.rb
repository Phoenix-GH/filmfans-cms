class CropableImageUploader < CarrierWave::Uploader::Base
  ...
  def zoom_and_crop
    model.update_column(:specification, {}) if (model.persisted? && model.image_file_changed?)
    if model.specification && model.respond_to?(:specification) && model.specification['crop_x'].present?
      manipulate! do |img|
        oriWidth = img[:width]
