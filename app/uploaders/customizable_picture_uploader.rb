...
class CustomizablePictureUploader < CarrierWave::Uploader::Base
  ...
  def zoom_and_crop
    if model.class.to_s == 'Picture' && model.respond_to?(:specification) && model.specification['crop_x'].present?
      manipulate! do |img|
        oriWidth = img[:width]
